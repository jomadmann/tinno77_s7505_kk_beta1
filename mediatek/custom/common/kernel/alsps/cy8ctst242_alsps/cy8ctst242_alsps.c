...
extern bool cy8ctst_msg2133(void);
//add by liyaohua
#define msg2133_alsp 
#ifdef msg2133_alsp 
extern int msg2133_open_sersor(int on);
extern int msg2133_get_sensor_state();
//end by liyaohuacy8ctst_get_sensor_state
#endif



//static void cy8ctst_alsps_work(struct work_struct *work)
void cy8ctst_alsps_work(struct work_struct *work)
{
	int rc = 0;
...
}else
{
	#ifdef msg2133_alsp 
	//add by liyaohua
	cy8ctst_alsps->ps = !msg2133_get_sensor_state();
	//end by liyaohua
        #endif
}
...
int  cy8ctst_alsps_get_openstate(void)
{
	struct cy8ctst_alsps_ctxt *cy8ctst_alsps = _cy8ctst_alsps;
	return cy8ctst_alsps->opened;
}
EXPORT_SYMBOL(cy8ctst_alsps_get_openstate);
...
long cy8ctst242_alsps_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int rc = 0;
	struct cy8ctst_alsps_ctxt *cy8ctst_alsps = file->private_data; 
	if(!cy8ctst_alsps) {
	...
	switch(cmd) {
		case ALSPS_SET_PS_MODE:
			...
			#ifdef msg2133_alsp 
			//add liyaohua
			msg2133_open_sersor(cy8ctst_alsps->enable);
			//end liyaohua
			#endif
			}
	...
	switch (command) {
		...
		case SENSOR_ENABLE:
			if((buff_in == NULL) || (size_in < sizeof(int))) {
				...
				#ifdef msg2133_alsp 
				//add liyaohua
				retmsg=msg2133_open_sersor(cy8ctst_alsps->enable);
				//end liyaohua
				#endif
				}
				rc = (ret < 0) ? rc : 0; 
