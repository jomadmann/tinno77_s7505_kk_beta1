...
void OV2659_night_mode(kal_bool enable)
{
	...
	{ 
		...
		OV2659_write_cmos_sensor(0x3a02 ,0x16);// 60Hz, maximum exposures, , reduce 1/6 , ->5 frames
		OV2659_write_cmos_sensor(0x3a03 ,0xf0);
				
		OV2659_write_cmos_sensor(0x3a14 ,0x16);
		OV2659_write_cmos_sensor(0x3a15 ,0xf0);
	...
	{   /* camera normal mode */
		OV2659_write_cmos_sensor(0x3a19, 0x3e);//gain ceiling 4x
   		...
   		OV2659_write_cmos_sensor(0x3a02 ,0x0b);// 60 maximum exposure . reduce 1/3, 10fps
...
static void OV2659InitialSetting(void)
{
	...
	OV2659_write_cmos_sensor(0x5027,0x19);
	OV2659_write_cmos_sensor(0x5028,0x34);
	...
	OV2659_write_cmos_sensor(0x5031,0xcc);
	OV2659_write_cmos_sensor(0x5032,0xe2);
	OV2659_write_cmos_sensor(0x5033,0xf6);
	...
	*/
	...
	OV2659_write_cmos_sensor(0x5077,0xbd); 
	...
	OV2659_write_cmos_sensor(0x5065,0x10); //10
	OV2659_write_cmos_sensor(0x5066,0x38); //0x38        
	OV2659_write_cmos_sensor(0x5067,0x28); //0x28
	...
	OV2659_write_cmos_sensor(0x506a,0x0d); //0x0d//18
	OV2659_write_cmos_sensor(0x506b,0x16); //0x16//28

	////////////////SDE/////////////////	
	OV2659_write_cmos_sensor(0x507e,0x35);
	OV2659_write_cmos_sensor(0x507f,0x30);
	...
	OV2659_write_cmos_sensor(0x3a11,0x50);//70
...
static void OV2659PreviewSetting(void)
{
	...
	if(!(OV2659_read_cmos_sensor(0x507b)&0xF0))
	{
		OV2659_write_cmos_sensor(0x5084,0x0c);//th1 1.5x
		OV2659_write_cmos_sensor(0x5085,0x1f);//1f//th2 4x
		OV2659_write_cmos_sensor(0x507e,0x35);//offset 1
		OV2659_write_cmos_sensor(0x507f,0x30);//0x18//offset 2
...
BOOL OV2659_set_param_effect(UINT16 para)
{
    ...
    switch (para)
    {
        case MEFFECT_OFF:
			 ...
			 //#ifdef OVT_AUTOUV
			 #if 1
				 OV2659_write_cmos_sensor(0x507e,0x35);//offset 1  , it must be same with PV setting[Ken]
        		 OV2659_write_cmos_sensor(0x507f,0x30);
        	 #endif
			 break;
...
BOOL OV2659_set_param_banding(UINT16 para)
{
    ...
    switch (para)
    {
        case AE_FLICKER_MODE_50HZ:
			...
			//OV2659_write_cmos_sensor(0x3a09, 0x5c);//5c
        ...
        case AE_FLICKER_MODE_60HZ:			
			spin_lock(&ov2659_drv_lock);
			 ...
			 OV2659_write_cmos_sensor(0x3a05, temp_reg|0x80);
			//OV2659_write_cmos_sensor(0x3a09, 0x4d);//4d
