...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	if (pmic_type == MT65XX_LED_PMIC_LCD_ISINK)
	{
		...
		{
#ifdef TINNO_ANDROID_S7505
			hwBacklightISINKTuning(0, PMIC_REGISTER, 0x3, 0);
			hwBacklightISINKTuning(1, PMIC_REGISTER, 0x3, 0);
			hwBacklightISINKTuning(2, PMIC_REGISTER, 0x3, 0);
			hwBacklightISINKTuning(3, PMIC_REGISTER, 0x3, 0);
			hwBacklightISINKTuning(4, PMIC_REGISTER, 0x3, 0);
			hwBacklightISINKTuning(5, PMIC_REGISTER, 0x3, 0);
#else
		...
		if (level) 
		{
			...
			if(level == ERROR_BL_LEVEL)
				level = tmp_level/17;
#ifndef TINNO_ANDROID_S7505
			hwPWMsetting(PMIC_PWM_0, level, div);
#else
			if(level<=ISINKs_BL_MAX_STEPPS){//up to 6 stepps.
				upmu_isinks_ch0_step(level);
				upmu_isinks_ch1_step(level);
				upmu_isinks_ch2_step(level);
				upmu_isinks_ch3_step(level);
				upmu_isinks_ch4_step(level);
				upmu_isinks_ch5_step(level);
			}
#endif
			upmu_top2_bst_drv_ck_pdn(0x0);
